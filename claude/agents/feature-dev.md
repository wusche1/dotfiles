...
1. Extract from the task: feature name (short, like "cot-batching") and source branch
2. Use the `setup-feature-branch` skill to create your isolated worktree
3. `cd` to your worktree path (returned by the skill)

## RULES

- **Work only in your worktree** (the path returned by setup-feature-branch)
- **Test after changes**: run the project's test command after significant changes
...
Run `git diff <source-branch>..HEAD` and carefully review:
